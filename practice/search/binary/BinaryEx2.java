...
public class BinaryEx2 {
	...
	static int binarySearch1(int key, int low, int high) { // low: 왼쪽, high: 오른쪽
		...
		if (low <= high) { // 찾는 데이터가 low, high 사이에 존재
			int middle = (low + high) / 2;

			if (low > high) { // 없는 값을 검색시 stackOverFlow 예외가 발생하지 않도록 예외처리
				return -1;
			}
			if (key == arr[middle]) { // 탐색성공
				return middle;
			}
			if (key < arr[middle]) {
				return binarySearch1(key, low, middle - 1); // 왼쪽 부분 탐색
			}
			if (key > arr[middle]) {
				return binarySearch1(key, middle + 1, high); // 오른쪽 부분 탐색
			}
		}
		return -1; // 탐색 실패
